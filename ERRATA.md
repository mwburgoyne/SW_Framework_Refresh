...
**Resolved:** the revised manuscript (R3, 2026-07-20, and the published version) quotes the current pyrestoolbox 3.6+ values (13.0% / 8.9% and matching sub-breakdowns), so the paper and this repository now agree.
...
`12-Generate_Figures.py` in this repository is fixed (S&W-original brine series constructed with the brine alpha; Gillespie exclusion added for H2), and regenerated figures match the text values. **Resolved:** the published version of the paper carries the regenerated figures with corrected legends.
