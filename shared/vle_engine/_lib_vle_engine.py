...
# Component database: All S&W 1992 gases + H2 (this work)
# Standardised 2026-07-23: Tc, Pc, omega, Tb from S&W 1992 Table 3 wherever
# tabulated there. H2 (absent from S&W): NIST. nC7+: see inline note below.
# (Pre-2026-07-23 the C2H6/C3H8/C5+ omegas followed the PhazeComp/Whitson
# library - see memory defensibility-sweep-s49. Full pipeline regenerated
# after the change.)
COMPONENTS = {
    # Water
    ...
    # nC7+: not in S&W Table 3. Tc/Pc/Tb verified within NIST WebBook
    # uncertainties 2026-07-23; omega = evaluated literature (Reid/Poling-class;
    # WebBook does not tabulate omega). Used by neither paper's results.
...
def kij_aq_c2h6_proposed(T_K: float, salinity_molal: float = 0.0) -> float:
    """C2H6: rational, n=95, MAE=0.0095 (MC-3 alpha; refit 2026-07-23 after omega standardisation)."""
    Tr = T_K / 305.40
    return (-1.2683 + Tr) / (0.3352 + 1.3035 * Tr)

def kij_aq_c3h8_proposed(T_K: float, salinity_molal: float = 0.0) -> float:
    """C3H8: rational, n=59, MAE=0.0022 (MC-3 alpha; refit 2026-07-23 after omega standardisation)."""
    Tr = T_K / 369.80
...
def kij_aq_c2h6_dropin(T_K: float, salinity_molal: float = 0.0) -> float:
    """C2H6: rational, n=95, MAE=0.0095 (S&W alpha; refit 2026-07-23 after omega standardisation)."""
    Tr = T_K / 305.40
    return (-1.2669 + Tr) / (0.1526 + 1.4335 * Tr)

def kij_aq_c3h8_dropin(T_K: float, salinity_molal: float = 0.0) -> float:
    """C3H8: rational, n=59, MAE=0.0023 (S&W alpha; refit 2026-07-23 after omega standardisation)."""
    Tr = T_K / 369.80
...
KIJ_NA: Dict[str, Optional[float]] = {
    ...
    'CH4': 0.4850,     # S&W 1992 Table 5
    'C2H6': 0.4920,    # S&W 1992 Table 5
...
class SWBinaryVLE:
    ...
    def calc_water_content_with_kij(self, T_K: float, P_Pa: float, kij_na: float,
        ...
        Runs the damped successive substitution twice (damp 0.4 and 0.2).
        Near the dew boundary (high T, P within a few multiples of Psat)
        the normalised iteration has multiple fixed points; when the two
        runs disagree by more than 0.5% relative, the answer is
        path-dependent and np.nan is returned instead of an arbitrary
        branch. Non-convergence also returns np.nan (previously the last
        iterate was returned silently). The damp=0.4 value is returned
        when unique, preserving the published kij_NA calibration.

        Args:
            T_K: Temperature in Kelvin
            P_Pa: Pressure in Pascals
            kij_na: Binary interaction parameter for non-aqueous phase
            max_iter: Maximum iterations (per damping pass)

        Returns:
            y_H2O: Water mole fraction in gas phase (np.nan if the
                   iteration fails to converge or is ambiguous)
        """
        y1, conv1 = self._water_content_ss(T_K, P_Pa, kij_na, 0.4, max_iter)
        if not conv1:
            return np.nan
        # Verification pass: smaller damping halves the step size, so allow
        # twice the iterations to converge on slow-but-unique points.
        y2, conv2 = self._water_content_ss(T_K, P_Pa, kij_na, 0.2, 2 * max_iter)
        if not conv2 or abs(y2 - y1) > 5e-3 * max(abs(y1), 1e-8):
            return np.nan
        return y1

    def _water_content_ss(self, T_K: float, P_Pa: float, kij_na: float,
                          damp: float, max_iter: int) -> Tuple[float, bool]:
