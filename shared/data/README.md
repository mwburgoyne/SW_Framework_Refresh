...
Compiled experimental gas solubility and water-content data for the main gases (CO₂, H₂S, CH₄, N₂, H₂) from the literature sources cited in the paper (sheet `QCd Data`). Columns include temperature, pressure, phase mole fractions (x_gas, y_H2O), NaCl molality (`Sal_m`), and source attribution. H₂ sources span Wiebe & Gaddy (1934) to Chabab et al. (2023); see the paper for the full source lists, quality assessment, and exclusions.
...
Implied Sechenov coefficients from the Blount & Price (1982) CH₄ brine dataset. Retained for the exclusion diagnostics reported in the paper (Section 5.5); this source is excluded from validation.
